...
class AdminCog(commands.Cog, name="Admin"):
    ...
    async def transaction_history(self, ctx, growid: str, limit: int = 10):
        ...
        try:
            # Gunakan get_transaction_history dari trx_manager
            ...
            for trx in transactions:
                value = (
                    ...
                    f"Items: {trx['items_count']}\n"
                    f"Price: {trx['total_price']:,} WL\n"
    ...
    async def reduce_stock(self, ctx, code: str, count: int):
        ...
        Usage: !reducestock <code> <count>
        Example: !reducestock DL 5
        """
        if not await self._check_admin(ctx):
            return
        ...
        try:
            ...
            # Get product info using ProductManagerService
            product = await self.product_service.get_product(code)
            if not product:
            ...
            try:
                # Get current stock count first
                current_stock = await self.product_service.get_stock_count(code)
                if current_stock < count:
                    raise ValueError(f"Not enough stock! Only {current_stock} items available")
                ...
                if len(stock_items) < count:
                    raise ValueError(f"Failed to get stock items. Available: {len(stock_items)}")
    
                # Reduce stock using reduce_stock method
                success = await self.product_service.reduce_stock(
                    product_code=code,
                    quantity=count,
                ...
                remaining_stock = await self.product_service.get_stock_count(code)
                ...
                embed.add_field(name="Reduced Amount", value=str(count), inline=True)
                embed.add_field(name="Remaining Stock", value=str(remaining_stock), inline=True)
                embed.add_field(
                    name="Change Summary", 
                    value=f"Before: {current_stock}\nRemoved: {count}\nAfter: {remaining_stock}", 
                    inline=False
                )
    ...
    async def dashboard(self, ctx):
        """Generate dashboard link for admin"""
        try:
            ...
            # Generate dashboard link dengan token
            dashboard_url = f"http://localhost:8000/admin/auth?token={token}"

            # Kirim link ke admin via DM
            try:
                await ctx.author.send(f"🔐 Your secure dashboard link (valid for 24 hours):\n{dashboard_url}")
                await ctx.message.delete()  # Hapus pesan command
                await ctx.send("✅ Dashboard link has been sent to your DM!", delete_after=5)
            except discord.Forbidden:
                await ctx.send("❌ I couldn't send you a DM. Please enable DMs from server members.", delete_after=5)

        except Exception as e:
            self.logger.error(f"Error generating dashboard link: {e}")
            await ctx.send("❌ An error occurred while generating dashboard link.", delete_after=5)
    ...
    async def check_balance(self, ctx, growid: str = None):
        ...
        try:
            ...
            # Get balance using BalanceManagerService
            ...
            # Balance info
            balance_text = balance.format()  # Use format() from Balance class
            ...
            # Get transaction history using TransactionManager
            transactions = await self.trx_manager.get_transaction_history(growid, 5)
            if transactions:
                trx_text = []
                for trx in transactions:
                    ...
                    trx_text.append(
                        ...
                        f"  Details: {trx['details']}\n"
                        f"  Balance: {trx['new_balance']}"
                    )
                embed.add_field(name="Recent Transactions", value="\n".join(trx_text), inline=False)
